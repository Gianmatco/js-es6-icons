...
in cui le options corrispondono ai vari tipi di icone 
(animal, vegetable, user). 
Quando l'utente seleziona un tipo dalla select, 
visualizzare solamente le icone corrispondenti.

BONUS
1- modificare la struttura dati fornita e
 valorizzare la proprietà "color" in modo dinamico: 
 generare in modo casuale un codice colore, 
 sapendo che la notazione esadecimale è formata dal simbolo "#" seguito da 
 6 caratteri alfanumerici compresi tra 0 e 9 e A e F.
...
//icone[2].name = 'figa';
//console.log(icone[2].name);
//console.log(icone[1].name);
...
function stampaicona(filtroIcona){
    container.innerHTML = '';
    filtroIcona.forEach((icona)=>{
...
const option = document.getElementById('opzioni');
//evento
option.addEventListener('change',function(){
    //console.log(this);
    const opzioniIcone = this.value;
    const filtroIcone = icone.filter((icona)=>{
        return icona.type === opzioniIcone || !opzioniIcone;
    })
    //console.log(opzioniIcone);
    //console.log(filtroIcone);
    //const typeArray = (opzioniIcone === '') ? icone : filtroIcone;
    stampaicona(filtroIcone);
})
